...
try:
    ...
    MEMORY_THRESHOLD = PROCESS_MEMORY_THRESHOLD
    SUSPICIOUS_NAMES = SUSPICIOUS_PROCESS_NAMES
    SUSPICIOUS_PATHS = SUSPICIOUS_PROCESS_PATHS
except ImportError:
    # Fallback if config not available
    LOG_FILE = "process_monitor.log"
    CPU_THRESHOLD = 80.0
    MEMORY_THRESHOLD = 1024
    SUSPICIOUS_NAMES = ["mimikatz", "psexec",
                        "ncat", "netcat", "nc.exe", "powercat"]
    SUSPICIOUS_PATHS = ["\\temp\\", "\\tmp\\",
...
def detect_suspicious_processes(processes, logger):
    ...
    for proc in processes:
        ...
        if reasons:
            ...
            logger.warning(
                f"Suspicious: {proc['name']} (PID {proc['pid']}) - {', '.join(reasons)}")
...
def print_system_summary(logger):
    ...
    print(
        f"Memory Used:  {memory.used / (1024**3):.2f} GB ({memory.percent}%)")
    ...
    try:
        ...
        if args.top:
            top_procs = get_top_processes(
                processes, count=args.top, sort_by="cpu")
            print_process_table(
                top_procs, f"TOP {args.top} PROCESSES (BY CPU)")

        if args.memory:
            top_procs = get_top_processes(
                processes, count=args.memory, sort_by="memory")
            print_process_table(
                top_procs, f"TOP {args.memory} PROCESSES (BY MEMORY)")

        if args.search:
            matches = search_processes(processes, args.search, logger)
            if matches:
                print_process_table(
                    matches, f"SEARCH RESULTS: '{args.search}'")
            else:
        ...
        if not any([args.top, args.memory, args.search, args.suspicious, args.summary]):
            ...
            print("\nExample usage:")
            print(
                "  Show top 10 CPU:      python system_admin/process_monitor.py --top 10")
            print(
                "  Show top 10 memory:   python system_admin/process_monitor.py --memory 10")
            print(
                "  Search for Chrome:    python system_admin/process_monitor.py --search chrome")
            print(
                "  Detect suspicious:    python system_admin/process_monitor.py --suspicious")
            print(
                "  System summary:       python system_admin/process_monitor.py --summary")
